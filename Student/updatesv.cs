        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (checkData())
            {
                ...
                try
                {
                    ...
                    string sql = "update sinhvien set ho_sv = @hosv, ten_sv = @tensv, ngay_sinh = @ngaysinh,k = @k,duong = @duong,huyen = @huyen,thanh_pho = @thanhpho ,khoa = @khoa,lop = @lop,chuong_trinh = @ct where ma_sv = @masv";
                    cmd = new NpgsqlCommand(sql, conn);
                    cmd.Parameters.Add("@masv", NpgsqlTypes.NpgsqlDbType.Varchar).Value = textBox5.Text;
                    ...
                    cmd.ExecuteNonQuery();
                    conn.Close();
                    MessageBox.Show("Update done", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                catch(Exception)
                {
